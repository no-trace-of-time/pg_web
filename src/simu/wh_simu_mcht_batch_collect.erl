...
%%%-------------------------------------------------------------------
-module(wh_simu_mcht_batch_collect).
-author("simonxu").
...
handle(Req, State) ->
  ...
  MchtOrderVals = [
    {actionUrl, pg_web:get_config_url(simu_mcht_batch_collect_final_url)}
    , {orderDesc, <<"测试交易"/utf8>>}
%%    , {orderId, "001"}
    , {merchId, ""}
  ...
  lager:debug("~n MchtBatchCollectVals = ~p~n", [MchtOrderVals]),

  %% mcht order parameters passed to dtl
  {ok, Body} = mcht_batch_collect_dtl:render(MchtOrderVals),
