...
%%%-------------------------------------------------------------------
-module(wh_simu_mcht_bankcard_verify_final).
-author("simonxu").
...
handle(Req, State) ->
  ...
  Body = try
           ...
           ModelMchtReqCollect = pg_protocol:out_2_in(pg_mcht_protocol_req_bankcard_verify, PostVals),
           {SignString, Signature} = pg_mcht_protocol:sign(pg_mcht_protocol_req_bankcard_verify, ModelMchtReqCollect),
           ...
           MchtOrderVals = xfutils:ext_req(Keys, PostVals) ++
             [
               {actionUrl, pg_web:get_config_url(txn_bankcard_verify_url)}
               , {merchId, MerchId}
           ...
           {ok, BodyResult} = mcht_bankcard_verify_final_dtl:render(MchtOrderVals),
           BodyResult
