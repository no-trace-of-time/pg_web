...
%%%-------------------------------------------------------------------
-module(wh_simu_mcht_batch_collect_final).
-author("simonxu").
...
handle(Req, State) ->
  ...
  Body = try
           ...
           lager:debug("MchtBatchCollectVals = ~p", [MchtOrderVals]),

           %% mcht order parameters passed to dtl
           {ok, BodyResult} = mcht_batch_collect_final_dtl:render(MchtOrderVals),
           BodyResult
